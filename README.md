...
The project reads structured configuration from the `config/` directory in YAML format.
...
- `application.yml` stores shared defaults
- `application.{env}.yml` stores environment-specific overrides
- The current environment is determined by `NODE_ENV`, defaulting to `dev`
